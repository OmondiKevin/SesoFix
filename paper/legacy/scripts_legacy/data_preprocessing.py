...
def split_dataset(dataset, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1, seed=42, synthetic_col='is_synthetic'):
    """
    Split dataset into train, validation, and test sets.
    If synthetic_col is present in the dataset columns, synthetic data is strictly
    isolated to the training set to prevent test/validation set contamination (leakage).
    ...
    Args:
        ...
        synthetic_col: Column name indicating if the row is synthetic
    ...
    # Check if we need to prevent leakage of synthetic data into validation/test splits
    if synthetic_col and synthetic_col in dataset.column_names:
        # Separate real and synthetic data
        real_dataset = dataset.filter(lambda x: not x[synthetic_col])
        synthetic_dataset = dataset.filter(lambda x: x[synthetic_col])
        
        # Split real data
        real_splits = split_dataset(real_dataset, train_ratio, val_ratio, test_ratio, seed, synthetic_col=None)
        
        # Concatenate synthetic data strictly to the training split
        if len(synthetic_dataset) > 0:
            train_dataset = concatenate_datasets([real_splits['train'], synthetic_dataset])
            # Shuffle the combined training set
            train_dataset = train_dataset.shuffle(seed=seed)
        else:
            train_dataset = real_splits['train']
            
        return DatasetDict({
            'train': train_dataset,
            'validation': real_splits['validation'],
            'test': real_splits['test']
        })
